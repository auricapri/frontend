...
import { Heart, SlidersHorizontal, ArrowLeft, ArrowRight, Tag, Truck } from 'lucide-react';
...
const ProductGrid: React.FC<ProductGridProps> = ({ 
  ...
  const { prefetchProductOnHover } = usePrefetch();
        ...
        {currentProducts.length === 0 && !isLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-x-12 gap-y-24">
            {!isLoading && currentProducts.map((p, index) => {
                 ...
                 const prefetchHandlers = prefetchProductOnHover(p);
                 
                 return (
                  <div 
                    key={p.id} 
                    onClick={() => onSelectProduct(p)} 
                    className="cursor-pointer group flex flex-col relative animate-in fade-in slide-in-from-bottom-4 duration-700"
                    {...prefetchHandlers}
                  >
