...
export const OrderReviewForm: React.FC<OrderReviewFormProps> = ({
  ...
  const [existingMedia, setExistingMedia] = useState<OrderReviewMedia[]>(existingReview?.media || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingReview) {
        ...
        if (!userId) {
          const ordersApi = (await import('../../api/orders.api')).OrdersApi;
          const orders = new ordersApi();
          const order = await orders.getByIdForReview(orderId);
          if (order?.user_id) {
