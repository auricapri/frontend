...
interface GalleryImage {
  ...
  variant?: {
    id: string;
    color_name: { pt: string; en?: string };
    color_hex: string;
    retail_price: number;
  ...
  product?: {
    ...
    slug: { pt: string; en?: string } | string;
  };
}

interface GalleryPageProps {
  onNavigate: (view: any, target?: string, product?: Product) => void;
...
export function GalleryPage({ onNavigate, onAddToCart, locale }: GalleryPageProps) {
  ...
  useEffect(() => {
    async function fetchGallery() {
      ...
      if (!galleryRows?.length) { setLoading(false); return; }

      const variantIds = [...new Set(galleryRows.map((r: any) => r.variant_id).filter(Boolean))];
      const productIds = [...new Set(galleryRows.map((r: any) => r.product_id).filter(Boolean))];
      ...
      setImages(galleryRows.map((r: any) => ({
        ...r,
        variant: r.variant_id ? variantMap[r.variant_id] : undefined,
        product: r.product_id ? productMap[r.product_id] : undefined,
      })));
      setLoading(false);
    }
    fetchGallery();
  ...
  const handleAddToCart = async (img: GalleryImage) => {
    if (!img.variant || !img.product || img.variant.stock_quantity < 1) return;
    setAddingId(img.id);
    onAddToCart({
      variant_id: img.variant.id,
      product_id: img.product.id,
      name: img.product.name as any,
      image: img.image_url,
      size: 'Tamanho único',
      color_name: img.variant.color_name as any,
  ...
  const handleNavigateToProduct = (img: GalleryImage) => {
        ...
        <div className="columns-2 md:columns-3 xl:columns-4 gap-3 md:gap-4">
          {images.map((img) => (
            <div
              ...
              onClick={() => handleNavigateToProduct(img)}
            >
              {/* Image */}
              <div className="relative overflow-hidden rounded-2xl md:rounded-3xl bg-neutral-100">
                ...
                {/* Hover overlay */}
                <div className={`absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent transition-opacity duration-500 rounded-2xl md:rounded-3xl
                ...
                `}>
                  <div className="absolute bottom-0 left-0 right-0 p-4 md:p-5">
                    ...
                    <button
                      ...
                      disabled={!img.variant || img.variant.stock_quantity < 1}
                      className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95
                        ...
                        ${(!img.variant || img.variant.stock_quantity < 1) ? 'opacity-50 cursor-not-allowed' : ''}
                      `}
                    >
                      <ShoppingBag className="w-3 h-3" strokeWidth={2.5} />
                      {addingId === img.id ? 'Adicionado!' : 'Adicionar ao carrinho'}
                    </button>
                  </div>
                </div>
              </div>
