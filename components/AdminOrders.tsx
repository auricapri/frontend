...
const AdminOrders: React.FC<AdminOrdersProps> = ({ orders, products = [], assets = [], onUpdateStatus, locale }) => {
  ...
  // --- PLP GENERATOR (Documento de Postagem) ---
  const generateAndDownloadPLP = (order: Order, metrics: { totalWeight: number, dimensions: string }) => {
      // 1. Prepare Data
      const sender = {
      ...
      const itemsHtml = order.items.map(item => `
          <tr>
              <td style="padding: 5px; border-bottom: 1px solid #eee;">${getLoc(item.name)} (${item.size || 'U'})</td>
              <td style="padding: 5px; border-bottom: 1px solid #eee;">${item.quantity}</td>
              <td style="padding: 5px; border-bottom: 1px solid #eee;">R$ ${(item.price).toFixed(2)}</td>
          </tr>
      `).join('');

      // 2. Build HTML Template (High Fidelity Label Style)
      const htmlContent = `
        <!DOCTYPE html>
        <html>
        <head>
            <title>PLP-${order.id.toUpperCase()}</title>
            <style>
                body { font-family: 'Courier New', monospace; padding: 20px; max-width: 800px; margin: 0 auto; border: 2px solid #000; }
                .header { text-align: center; border-bottom: 2px solid #000; padding-bottom: 20px; margin-bottom: 20px; }
                .row { display: flex; justify-content: space-between; margin-bottom: 15px; }
                .box { border: 1px solid #000; padding: 10px; width: 48%; }
                .title { font-weight: bold; font-size: 12px; text-transform: uppercase; margin-bottom: 5px; display: block; }
                .content { font-size: 14px; font-weight: bold; }
                .barcode { text-align: center; margin: 20px 0; border: 1px dashed #000; padding: 10px; }
        ...
        <body>
            <div class="header">
                <h1>DOCUMENTO DE POSTAGEM (PLP)</h1>
                <p>AUTORIZAÇÃO DE ENVIO V19.0.4</p>
                <div style="font-size: 24px; font-weight: bold; margin-top: 10px;">${order.internal_logistics?.selected_carrier || 'TRANSP. PADRÃO'}</div>
            </div>
            ...
            <div class="row">
                ...
                <div class="box">
                    ...
                    <div class="content">
                        ${recipient.logradouro}, ${recipient.numero || ''}<br/>
                        ${recipient.bairro || ''} - ${recipient.localidade}/${recipient.uf}<br/>
                        CEP: ${recipient.cep}
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="box">
                    <span class="title">DADOS LOGÍSTICOS</span>
                    <div class="content">
                        PESO BRUTO: ${metrics.totalWeight}g<br/>
                        DIMENSÕES: ${metrics.dimensions}<br/>
                        VOLUME: 1/1
                    </div>
                </div>
                <div class="box">
                    <span class="title">DADOS DO PEDIDO</span>
                    <div class="content">
                        ID: #${order.id.slice(0, 8).toUpperCase()}<br/>
                        DATA: ${new Date().toLocaleDateString()}<br/>
                        VALOR DECLARADO: R$ ${order.total.toFixed(2)}
                    </div>
                </div>
            </div>

            <div style="margin-top: 20px; border-top: 2px solid #000; padding-top: 10px;">
                <span class="title">CONTEÚDO DA EMBALAGEM</span>
                <table>
            ...
            <div class="barcode">
                <h1>||||| |||| || |||||| |||| ||||</h1>
                <p>PLP-SEQ-${Date.now()}-${order.id.slice(0,4)}</p>
            </div>

            <div style="font-size: 10px; text-align: center; margin-top: 20px;">
                Este documento deve ser anexado externamente à embalagem.
                Gerado via Auricapri ERP.
            </div>
        </body>
        </html>
      `;
      ...
      a.href = url;
      a.download = `PLP-${order.id.slice(0,8)}.html`; // Downloads as an HTML file acting as the doc
  ...
  const handleGenerateDoc = () => {
      ...
      const metrics = calculateLogisticsMetrics(selectedOrder);
      generateAndDownloadPLP(selectedOrder, metrics);
                                  ...
                                  {!isDocGenerated ? (
                                      ...
                                      <div className="flex gap-4 items-center">
                                          ...
                                          <div>
                                              ...
                                              <button onClick={handleGenerateDoc} className="text-[9px] underline">Baixar novamente</button>
                                          </div>
                                      </div>
                                  )}
                              </div>
