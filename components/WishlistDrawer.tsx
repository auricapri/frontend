...
const WishlistDrawer: React.FC<WishlistDrawerProps> = ({ 
  ...
  const handleShareLink = () => {
    // Simulated public gift link
    const shareUrl = `${window.location.origin}/wishlist/gift-${Math.random().toString(36).substr(2, 9)}`;
              ...
              <div className="pt-10 border-t border-neutral-200">
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      {copiedLink ? <ShoppingCart className="w-4 h-4 text-green-500" /> : <Link2 className="w-4 h-4" />}
                      <span className="text-[10px] font-black uppercase tracking-widest">{copiedLink ? 'Copied' : 'Gift Link'}</span>
                    </button>
                    <button 
