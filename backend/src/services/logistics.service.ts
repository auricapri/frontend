...
import type { InternalLogisticsInfo, AddressData } from '../repositories/orders.repository.js';
...
export class LogisticsService {
  ...
  async fetchAddressByCep(cep: string): Promise<ViaCepResponse | null> {
    ...
    try {
      const cacheKey = `viacep:${cleanCep}`;
      if (this.redis) {
        const cached = await this.redis.get(cacheKey);
        if (cached) {
          try {
            return JSON.parse(cached) as ViaCepResponse;
          } catch {
      ...
      const response = await this.fetchJson<ViaCepResponse>(`https://viacep.com.br/ws/${cleanCep}/json/`, 4000);
      ...
      if (this.redis) {
        await this.redis.setex(cacheKey, 24 * 60 * 60, JSON.stringify(data));
      }

      return data;
    } catch (error) {
      logger.warn('ViaCEP API error', { error });
      return null;
    }
  }
  ...
  private async saveQuotesToDatabase(input: FreightCalculationInput, quotes: FreightQuote[]): Promise<void> {
    try {
      ...
      logger.warn('Error saving freight quotes to cache', { error });
    }
  }
